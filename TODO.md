...
- `TileFactory.update_heads_and_tails` (`tiles.py`) only ever sets `is_tail = True` for a tile that already has a `link_prev` -- a fully standalone tile (no links at all) comes out `is_head=True`, `is_tail=False`. Found and documented, not fixed (M6's `ACTION_PLAN.md` decision excludes redesigning tiles/stretches) -- see `tests/test_tiles.py::test_tiles_are_unlinked_given_the_non_adjacent_layout`. More found during the type hints sweep, 2026-09-24: the same method's last branch marks a *body* tile (linked on both sides) `is_tail = True` as well; `Tile.__init__` sets `link_next_type`, which nothing reads, while `link_consecutive_tiles` sets an attribute `link_type` that `__init__` never creates; and `is_straight_jsr_tile` loops over `next_info.leaps_from.infos`, which is `None` when nothing leaps to the next tile -- a possible crash.
...
- `core/cpu.py` has its own copy of `signed()`, identical to the one in `util.py`. Import it from `util` instead, and remove the copy? Found during the type hints sweep, 2026-09-24.
- `Display.save_hires_bytes`/`load_hires_bytes` (`core/apple.py`) call `self.pickle.dump(...)`/`self.pickle.load(...)`, but `self.pickle` is the class's own `pickle(pickler)` method, so both would crash with `AttributeError`. The only caller is `RecordedKeys.press_keys` in `debug/checkpoints.py` -- Robotron-specific and unused (corrected 2026-09-24: an earlier version of this entry wrongly named `KeyScript`). Decide: fix (use the `pickle` module), or remove both; they would go naturally with the Robotron removal below. Found during the type hints sweep, 2026-09-24.
...
- `core/hooks.py` records memory accesses as nested tuples, `((cycles, last_PC), reads, write)`, unpacked by position all over `MemAccessCollector` and `TimeMachine`. Refactor into small `dataclass`es (a write: address, old value, new value; a read: address, value), and `mem_access_colors`'s `kind` (`'reads'`/`'writes'`/`'pcs'`) into an `enum`. `probotron` consumes the resulting tables, so check it along. Same pattern in `debug/disassembler.py`: the addressing modes return small dicts (`operand`, `operand_address`, `memory`, ...), typed for now by the `OperandInfo` alias -- a `dataclass` would replace both. Surfaced during the type hints sweep, 2026-09-24.
- `TimeMachine` in the window seems not to work (observed in `make boot-lode-runner`, 2026-09-24): Stopped + left arrow prints "restore", but the PC does not change, and holding the key down steps only once. (a) Confirmed 2026-09-24: `scripts/boot_lode_runner.py` creates its `Emulator` without `time_machine=True`, so the `TimeMachine` never records, and `restore_prev_state` returns at once -- after `on_left` has already printed "restore". Leads, not yet checked: (b) nothing calls `pygame.key.set_repeat`, so a held key sends one event; (c) the status line is not refreshed after a restore, so it would show the old PC even if the restore worked. Revisit with the monitor.

- Remove the Robotron-specific code from `papple2` (decided 2026-09-24): `Labels.add_standard_labels` in `debug/labels.py` hard-codes about 50 Robotron labels (`atariPresents`, `roboNoises`, ...), and `debug/checkpoints.py` has three Robotron classes nobody uses -- `RecordedKeys` (cycle counts from one Robotron run, and the only caller of the broken `save_hires_bytes`), `PrintCharTester` and `RandomTesterCheckpoint` (Robotron addresses; only mentioned in a commented-out line in `core/emulator.py`). Move what `probotron` still needs over there, delete the rest. Also in `debug/tiles.py`: `is_branch_always_tile` hard-codes the Robotron address `$51b6`, and the module docstring still points to `examples/Robotron/workbench.py` (now in `probotron`). Note along the way: the label table has `waitKbd` twice (`$4242` and `$453d`), so a disassembly cannot tell them apart.
