...
- `TimeMachine` in the window seems not to work (observed in `make boot-lode-runner`, 2026-09-24): Stopped + left arrow prints "restore", but the PC does not change, and holding the key down steps only once. Leads, not yet checked: (a) the `TimeMachine` only records when the `Emulator` is created with `time_machine=True` -- otherwise `restore_prev_state` returns at once, after `on_left` has already printed "restore"; (b) nothing calls `pygame.key.set_repeat`, so a held key sends one event; (c) the status line is not refreshed after a restore, so it would show the old PC even if the restore worked. Revisit with the monitor.
